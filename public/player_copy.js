...
import { MotionMixer } from './MotionMixer.js';
...
export const player = (() => {
  ...
  class Player {
    constructor(params) {
      ...
      this.params_ = params;
      this.mesh_ = null;
      this.motionMixer_ = null; // MotionMixer 인스턴스로 대체
    ...
    OnKeyDown_(event) {
      ...
      switch (event.code) {
        ...
        case 'KeyJ':
          ...
          const isRangedWeapon = this.equippedWeapon_ && this.equippedWeapon_.type === 'ranged';
          let attackAnimation = isRangedWeapon ? 'Shoot_OneHanded' : 'SwordSlash';

          if (isMoving && !isRangedWeapon) {
          ...
          if (
            ...
            this.attackDirection_.copy(moveDir);
            this.SetAnimation_(attackAnimation); // 무기 타입 및 이동 상태에 따른 애니메이션 사용
    ...
    LoadModel_() {
      ...
      loader.load('Suit_male.gltf', (gltf) => {
        ...
        model.traverse((c) => {
          ...
          const skeleton = model.getObjectByProperty('isSkeleton', true);
          if (skeleton) {
            console.log("Actual Bone Names from Three.js Model (from player.js):",
            skeleton.bones.map(bone => bone.name));
   18       }
        this.motionMixer_ = new MotionMixer(model, gltf.animations);
        this.motionMixer_.createCompositeAction('walkAttack', 'Walk', 'SwordSlash');

        this.motionMixer_.mixer.addEventListener('finished', (e) => {
          if (e.action.getClip().name === 'SwordSlash' || e.action.getClip().name === 'Shoot_OneHanded') {
            ...
            if (this.keys_.j_key) {
                this.attackCooldownTimer_ = this.attackCooldown_;
                this.motionMixer_.play(this.currentAnimationName_, 0.1);
              } else {
    ...
    EquipItem(item) {
      ...
      if (this.motionMixer_) {
        this.motionMixer_.play(name, 0.3);
      }
    }
    ...
    Update(timeElapsed, rotationAngle = 0) {
      ...
      if (this.isDead_) {
        ...
        if (this.motionMixer_) {
          this.motionMixer_.mixer.update(timeElapsed);
        }
        return;
      }
      ...
      if (this.isAttacking_) {
        ...
        if (this.isAerialAttacking_) {
          const currentAnimationName = this.currentAnimationName_;
          let currentAnimationTime = 0;
          if (this.motionMixer_ && this.motionMixer_.activeAction) {
              if (this.motionMixer_.activeAction.isComposite) {
                  currentAnimationTime = this.motionMixer_.activeAction.upper.time;
              } else {
                  currentAnimationTime = this.motionMixer_.activeAction.time;
              }
          }
      ...
      if (this.isDead_) {
          ...
          // 공격 애니메이션은 OnKeyDown_에서 이미 설정되었으므로 여기서는 상태만 유지
          // 필요하다면 여기에 공격 애니메이션의 특정 프레임 로직을 추가할 수 있음
      ...
      if (this.motionMixer_) {
        this.motionMixer_.update(timeElapsed);
      }
